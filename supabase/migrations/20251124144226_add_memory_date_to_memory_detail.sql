-- Migration: Add memory_date to get_memory_detail function
-- Description: Includes memory_date field in memory detail RPC response
...
BEGIN
  ...
  -- Return the result with memory_date
  RETURN QUERY SELECT
    v_memory.id,
    v_memory.user_id,
    v_memory.title,
...
'Returns all fields needed for memory detail view including photos, videos, location data, '
'related memories, audio fields for stories, and memory_date for user-specified date.';
